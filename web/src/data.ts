...
export async function resolveCompound(id: string, disease: Disease = "pd"): Promise<Compound | null> {
  const live = await tryLive<Compound>(`/resolve?id=${encodeURIComponent(id)}${dq(disease, "&")}`);
  if (live) return live;
